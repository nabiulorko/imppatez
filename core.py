...
PROJECT_NAME = "IMPPATez"
PROJECT_DESCRIPTION = (
    "Natural product informatics and molecular docking workflow helpers."
)
PROJECT_VERSION = "0.2.0"

OPTIONAL_DEPENDENCIES = {
    "rdkit": "core cheminformatics and molecule preparation",
    "prody": "protein parsing, receptor cleanup, and ligand detection",
    "meeko": "PDBQT export for AutoDock Vina-compatible engines",
    "dimorphite_dl": "pH-aware protonation fallback",
    "gemmi": "mmCIF to PDB conversion",
    "requests": "remote API access for pKa/PoseView services",
    "numpy": "geometry and structural analysis",
    "PIL": "PNG stamping for interaction diagrams",
...
def runtime_report() -> dict:
    ...
    dependencies = {
        ...
        for name, purpose in OPTIONAL_DEPENDENCIES.items()
    }
    return {
...
def missing_dependency_names(required: tuple[str, ...] | None = None) -> list[str]:
    report = runtime_report()["dependencies"]
    names = required or tuple(report)
    return [name for name in names if not report.get(name, {}).get("available", False)]
...
def _find_ionizable_sites(mol):
    ...
    if not _IONIZABLE_SITES_COMPILED:
        _IONIZABLE_SITES_COMPILED = _compile_ionizable_sites()
